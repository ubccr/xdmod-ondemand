...
If you are on Ubuntu, run the two commands below to install the Python virtual
environment package for your version of Python 3. If you are not on Ubuntu,
these two commands should be skipped.
...
script can be run again, you will need to recreate the virtual environment and
reinstall the `xdmod-ondemand-export` package:
...
The `xdmod-ondemand-export` command can be run manually as follows, replacing
`[OPTIONS]` with one or more of the options described below:

```
su -c '/home/xdmod-ondemand-export/venv/bin/xdmod-ondemand-export [OPTIONS]' -s /bin/bash xdmod-ondemand-export
```
